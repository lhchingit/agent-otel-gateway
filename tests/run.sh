...
expect "^ai_agent_token_usage_total${L}agent=\"codex\"${L}session_id=\"[0-9]{18,19}\"" "codex: session_id synthesised from datapoint start time"
expect "^ai_agent_token_usage_total${L}agent=\"pi\"${L}session_id=\"[0-9]{18,19}\"" "pi: session_id synthesised from datapoint start time"
expect "^ai_agent_tool_call_count_total${L}agent=\"codex\"${L}session_id=\"[0-9]{18,19}\"" "codex: synthesised session_id also on sums"
